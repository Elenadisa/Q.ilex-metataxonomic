...
letters <- multcompLetters4(anova, tukey)$Site

letters_df <- data.frame(Site = names(letters$Letters), Letter = as.character(letters$Letters))
...
```{r genusbarplot}
genus_count_df_bact <- merge(genus_count_df_bact, letters_df, by = "Site")
...
alpha_bact <- alpha_diversity_plot(physeq.clean, "Site", metrics, comparisons, stat_metrics, asterisk = TRUE, " ")
alpha_bact
...
beta_bact <- plot_distance(physeq.css, pcoa_bray, "Site", elipse = TRUE) + annotate("text" ,x=0.2,y=0.43, size = 5, label = bray_permanova_text) 
...
top_sample_bact <- plot_abundance_by_sample(genus_top_filter.df, "SampleID", "Rel_Abundance", "Genus", "SampleID") + facet_wrap(~Site, scales = "free") + theme(strip.text = element_text(size = 14)) 
...
top_bact <- plot_abundance_by_taxa(top_genus_rel_sumarize, "Genus", "Relative_Abundance", "Site") 
top_bact
...
topsite_bact <- plot_abundance_by_taxa(top_20_genus_rel_sumarize, "Genus", "Relative_Abundance", "Site")
topsite_bact
```
...
meco_bact <- phyloseq2meco(genus_rel_ps)
...
textdf_bact <- obtain_significance_df(sig_pathways, abund_t_bact)
fta_bpl_bact <- fta_abundance_barplot(abund_t_bact, textdf_bact, "Site")
fta_bpl_bact
```