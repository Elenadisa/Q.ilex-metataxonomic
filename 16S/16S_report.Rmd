...
```{r echo=FALSE}
#load own functions
source("C:MAKE/PATH/Q.ilex-metataxonomic/functions.R")
```
...
It is proposed to perform an analysis of the diversity and composition of microbial communities in each soil type using techniques such as amplicon sequencing (e.g., using markers like the 16S rRNA gene for bacteria).
  
#Load Data  
Sequences were analyse with the software QIIME2. Now we import the generated dataset to perform downstream analysis.  
  
```{r }
physeq <-qza_to_phyloseq(features="taxonomy/trimmed2/final_FeatureTable.qza", 
                         tree="phylogeny/trimmed2/rooted-tree.qza", 
                         taxonomy="taxonomy/trimmed2/taxonomy.qza", 
...
letters <- multcompLetters(tukey$Site[, "p adj"])$Letters
letters <- as.data.frame(letters)
letters$Site <- rownames(letters)

#Mean and SD Genera number by site
genus_count_df_bact <- genus_count_sample_df%>%
  group_by(Site) %>%
  summarise(Genus_mean = mean(Genus_nb), SD = sd(Genus_nb))

kable(genus_count_df_bact, row.names = FALSE, caption = "Number of genera identified per experimental condition. The table presents the total number of genera detected in each experimental Site, without considering their abundances. This provides an overview of the taxonomic diversity present in each condition based solely on the presence of genera.")
```

```{r genusbarplot}
genus_count_df_bact <- merge(genus_count_df_bact, letters, by = "Site")
genus_count_df_bact$Type <- rep("Prokaryote", nrow(genus_count_df_bact))

ggplot(genus_count_df_bact, aes(x=Site, y=Genus_mean)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(aes(ymin=Genus_mean, ymax=Genus_mean+SD), width=.2, position=position_dodge(.9)) +
  geom_text(aes(label = letters), size = 6, position="jitter", vjust = -1)
```
...
comparisons <-list(c("Abla", "Ronda"), c("Abla", "Somiedo"), c("Ronda", "Somiedo"))
...
alpha_diversity_plot(physeq.clean, "Site", metrics, comparisons, stat_metrics, asterisk = TRUE, " ")

#alpha diversity table
alpha.tb <- estimate_richness(physeq.clean, measures=metrics)
alpha.tb$Site <- physeq.clean@sam_data$Site
write.table(alpha.tb, "prokaryote_alpha_div_tb.tsv", sep = "\t", quote = FALSE)
kable(alpha.tb)
```
...
```{r Pairwise1}
pairwise_bray <- pairwise.adonis2(bray_distance ~ Site, data = meta)
pairwise_bray$Ronda_vs_Somiedo
```

*Ronda vs Abla*

```{r Pairwise2}
pairwise_bray$Ronda_vs_Abla
```

*Somiedo vs Abla*

```{r Pairwise3}
pairwise_bray$Somiedo_vs_Abla
```
...
plot_distance(physeq.css, pcoa_bray, "Site", elipse = TRUE) + annotate("text" ,x=0.2,y=0.43, label = bray_permanova_text) 
```
...
ps.taxa.sub <- subset_samples(ps_genus, Site %in% c("Ronda", "Somiedo"))
ps.taxa.sub <- prune_taxa(taxa_sums(ps.taxa.sub) > 1, ps.taxa.sub)

#Convert to relative abundance according to a taxonomic level -> Genus
genus_rel_abun <- relative_abundance_by_taxa(ps.taxa.sub, "Genus")
#physeq object
...
```{r Top20, fig.width=8, fig.height=6}
#Filter top 20
...
plot_abundance_by_sample(genus_top_filter.df, "SampleID", "Rel_Abundance", "Genus", "SampleID") + facet_wrap(~Site, scales = "free")
```
...
plot_abundance_by_sample(genus_1_filter.df, "SampleID", "Rel_Abundance", "Genus", "SampleID") + facet_wrap(~Site, scales = "free")
```
...
plot_abundance_by_taxa(top_genus_rel_sumarize, "Genus", "Relative_Abundance", "Site")
```
...
plot_abundance_by_taxa(top_20_genus_rel_sumarize, "Genus", "Relative_Abundance", "Site")
```
...
meco_bact <- phyloseq2meco(ps.taxa.sub)
t1_bact <- trans_func$new(meco_bact)

#Search in faproxtax
t1_bact$cal_spe_func(prok_database = "FAPROTAX")
#obtain abundance
t1_bact$cal_spe_func_perc(abundance_weighted = TRUE)
#obtain percentage
t1_bact$trans_spe_func_perc()

# clone meco_bact for differential test
tmp_bact <- clone(meco_bact)

# we need assign the table to taxa_abund manually
tmp_bact$taxa_abund$func <- as.data.frame(t(t1_bact$res_spe_func_perc), check.names = FALSE)
# use trans_diff class to perform differential test
t2_bact <- trans_diff$new(dataset = tmp_bact, method = "wilcox", group = "Site", taxa_level = "func", alpha = 0.05)

sig_pathways <- t2_bact$res_diff[t2_bact$res_diff$P.adj < 0.05,]
pathway_l <- unique(sig_pathways$Taxa)
abund_t_bact <- t2_bact$res_abund[t2_bact$res_abund$Taxa %in% pathway_l, ]
abund_t_bact$Taxa <- as.character(abund_t_bact$Taxa) 

textdf_bact <- obtain_significance_df(sig_pathways, abund_t_bact)
fta_abundance_barplot(abund_t_bact, textdf_bact, "Site")
```

```{r DAA-FTA-moresign, fig.width=8}
sig_pathways <- t2_bact$res_diff[t2_bact$res_diff$P.adj < 0.003,]
pathway_l <- unique(sig_pathways$Taxa)
abund_t_bact <- t2_bact$res_abund[t2_bact$res_abund$Taxa %in% pathway_l, ]
abund_t_bact$Taxa <- as.character(abund_t_bact$Taxa) 
